...
- [x] `lembretes_enviados` table has `risco_noshow` INTEGER column (already existed)
- [x] `lembretes_enviados` table has `mensagem_enviada` TEXT column (already existed)
- [x] N8N workflow INSERT includes `risco_noshow` with value from Prepara Envio WhatsApp
- [x] N8N workflow INSERT includes `mensagem_enviada` with value from Prepara Envio WhatsApp
