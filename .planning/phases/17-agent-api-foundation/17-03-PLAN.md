...
must_haves:
  truths:
    - "flexibleDateSchema accepts ISO 8601 with timezone offset"
    - "flexibleDateSchema accepts local datetime without offset"
    - "flexibleDateSchema accepts date-only strings"
    - "All date formats are transformed to TZDate in clinic timezone"
    - "Agent appointment schemas use flexible date parsing"
  artifacts:
    - path: "src/lib/validations/agent-schemas.ts"
      provides: "Flexible Zod schemas for agent API validation"
      exports: ["flexibleDateSchema", "flexibleDateTimeSchema", "agentAppointmentSearchSchema"]
  key_links:
    - from: "src/lib/validations/agent-schemas.ts"
      to: "@date-fns/tz"
      via: "TZDate transform"
      pattern: "TZDate"
    - from: "src/lib/validations/agent-schemas.ts"
      to: "src/lib/calendar/time-zone-utils.ts"
      via: "CLINIC_TIMEZONE constant"
      pattern: "CLINIC_TIMEZONE"
---

<objective>
Create flexible Zod validation schemas for agent API inputs.

Purpose: N8N AI Agent may send dates in various formats (ISO 8601 with offset, local datetime, date-only). These schemas accept all valid formats and transform them to TZDate in the clinic timezone for consistent storage and display.

Output:
- `src/lib/validations/agent-schemas.ts` with flexible date schemas and agent-specific validation schemas
</objective>
...
<task type="auto">
  <name>Task 1: Create flexible date validation schemas</name>
  <files>src/lib/validations/agent-schemas.ts</files>
  <action>
Create `src/lib/validations/agent-schemas.ts` with flexible date parsing:
...
- flexibleDateTimeSchema handles all ISO 8601 variants
- All dates transformed to TZDate in `America/Sao_Paulo` (clinic timezone)
- Schemas prepared for all Phase 18-20 tools (query, write, complex)
  </action>
  <verify>
Run `npx tsc --noEmit` - no type errors.
Check exports count: `grep -c "^export" src/lib/validations/agent-schemas.ts` should be 10+.
Test date parsing: Create a quick test file to verify flexibleDateTimeSchema works.
  </verify>
  <done>agent-schemas.ts exists with flexibleDateTimeSchema, flexibleDateSchema, and all agent-specific validation schemas with proper TypeScript types exported</done>
</task>
...
2. Check file has flexibleDateTimeSchema: `grep "flexibleDateTimeSchema" src/lib/validations/agent-schemas.ts`
3. Check TZDate import: `grep "TZDate" src/lib/validations/agent-schemas.ts`
4. Count exported schemas: `grep -c "^export const" src/lib/validations/agent-schemas.ts` should be 10+
5. Count exported types: `grep -c "^export type" src/lib/validations/agent-schemas.ts` should be 10+
</verification>

<success_criteria>
- src/lib/validations/agent-schemas.ts exists
- flexibleDateTimeSchema accepts ISO 8601 with offset, UTC (Z), and local formats
- flexibleDateSchema accepts date-only format (YYYY-MM-DD)
- Both date schemas transform to TZDate in America/Sao_Paulo timezone
- All agent-specific schemas exported with corresponding TypeScript types
- TypeScript compilation passes
</success_criteria>

<output>
After completion, create `.planning/phases/17-agent-api-foundation/17-03-SUMMARY.md`
</output>
